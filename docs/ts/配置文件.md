...
include/exclude 的作用也很好理解，我们可以指定编译某些文件，或者指定排除某些文件

exclude 是有默认值的，如果我们没有设置 exclude ，那其默认值为 node_modules 、bower_components、jspm_packages 和编译选项 outDir 指定的路径。

三者的优先级是`files > exclude > include`
...
但是 要想支持这个特性需要 Visual Studio 2015， TypeScript1.8.4 以上并且安装 atom-typescript 插件。
